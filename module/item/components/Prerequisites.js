...
export class ItemPrerequisite extends Prerequisite {
    ...
    qualifies(actorDoc) {
        if (super.qualifies(actorDoc)) {
            ...
            return qualifier != null && item != null && (qualifier.unary ? qualifier.predicate(item) : this.qualifyItemValues(qualifier, item));
        }
        return false;
    }
...
export class ActorPrerequisite extends Prerequisite {
    ...
    qualifies(actorDoc) {
        if (super.qualifies(actorDoc)) {
            ...
            const qualifier = PrimeItemTables.qualifierForKey(this.qualifier);
            return qualifier != null && qualifier.unary ? qualifier.predicate(object) : qualifier.predicate(object, this.value);
        }
        return false;
    }
